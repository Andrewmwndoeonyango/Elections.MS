...
class Command(BaseCommand):
    help = 'Populate the database with complete Kenyan election data'

    def handle(self, *args, **options):
        self.stdout.write('Starting to populate Kenyan election data...')
        ...
        # Create counties and their administrative units
        self.create_counties_and_units()

        # Create candidates for all positions
        self.create_candidates()

        self.stdout.write(self.style.SUCCESS(
            'Successfully populated all Kenyan election data!'))

    def create_parties(self):
        parties_data = [
            ('Jubilee Party', 'JP', 'A center-right political party in Kenya'),
            ('Orange Democratic Movement', 'ODM',
             'A center-left political party in Kenya'),
            ('United Democratic Alliance', 'UDA',
             'A political party in Kenya associated with William Ruto'),
            ('Wiper Democratic Movement', 'WDM',
             'A political party in Kenya led by Kalonzo Musyoka'),
            ('Amani National Congress', 'ANC',
             'A political party in Kenya led by Musalia Mudavadi'),
    ...
    def create_counties_and_units(self):
        # Kenyan counties with their codes (47 counties)
        counties_data = [
            ('Mombasa', '001'), ('Kwale', '002'), ('Kilifi',
                                                   '003'), ('Tana River', '004'), ('Lamu', '005'),
            ('Taita Taveta', '006'), ('Garissa', '007'), ('Wajir',
                                                          '008'), ('Mandera', '009'), ('Marsabit', '010'),
            ('Isiolo', '011'), ('Meru', '012'), ('Tharaka Nithi',
                                                 '013'), ('Embu', '014'), ('Kitui', '015'),
            ('Machakos', '016'), ('Makueni',
                                  '017'), ('Nairobi', '018'), ('Kajiado', '019'),
            ('Kericho', '020'), ('Bomet', '021'), ('Kakamega',
                                                   '022'), ('Vihiga', '023'), ('Bungoma', '024'),
            ('Busia', '025'), ('Siaya', '026'), ('Kisumu',
                                                 '027'), ('Homa Bay', '028'), ('Migori', '029'),
            ('Kisii', '030'), ('Nyamira', '031'), ('Nyeri',
                                                   '032'), ('Kirinyaga', '033'), ('Muranga', '034'),
            ('Kiambu', '035'), ('Turkana', '036'), ('West Pokot',
                                                    '037'), ('Samburu', '038'), ('Trans Nzoia', '039'),
            ('Uasin Gishu', '040'), ('Elgeyo Marakwet', '041'), ('Nandi',
                                                                 '042'), ('Baringo', '043'), ('Laikipia', '044'),
            ('Nakuru', '045'), ('Narok', '046'), ('Nyandarua', '047'),
        ]
        ...
        self.stdout.write('Created counties and administrative units')

    def create_county_units(self, county):
        # Generate constituencies for each county (approximately 290 total)
        ...
        for i in range(1, num_constituencies + 1):
            ...
            # Create wards for each constituency (approximately 5 wards per constituency)
            num_wards = random.randint(4, 6)
    ...
    def get_constituency_count(self, county_name):
        # Approximate constituency counts for major counties
        major_counties = {
            'Nairobi': 17, 'Kakamega': 12, 'Kiambu': 12, 'Meru': 9, 'Bungoma': 9,
            'Nakuru': 11, 'Kisumu': 8, 'Mombasa': 6, 'Kilifi': 7, 'Machakos': 8,
            'Kitui': 8, 'Muranga': 7, 'Nyeri': 6, 'Uasin Gishu': 6,
        }

        return major_counties.get(county_name, random.randint(3, 8))
    ...
    def create_presidential_candidates(self, parties):
        ...
        self.stdout.write('Created presidential candidates')
    ...
    def create_women_rep_candidates(self, parties):
        ...
        self.stdout.write('Created women representative candidates')
    ...
    def create_mca_candidates(self, parties):
        ...
        self.stdout.write('Created MCA candidates')
