...
print('=== PROCESSING MURANGA AND KIAMBU ===')

updated = 0
created = 0
errors = 0

with open('central_counties_data.csv', 'r', encoding='utf-8') as file:
    reader = csv.DictReader(file)
    
    for row in reader:
        county_name = row['county_name'].strip().title().replace("'A", 'A')
        constituency_name = row['constituency_name'].strip()
        ward_name = row['ward_name'].strip()
        ward_code = row['ward_code'].strip()
        
        try:
            # Get existing county (handle Muranga special case)
            if 'MURANG' in county_name:
                county = County.objects.get(name__iexact='Muranga')
            else:
